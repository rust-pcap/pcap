...
- `Capture::min_to_copy` is now on `Capture<Active>` and returns a `Result`. pcap accepts a
  mintocopy value only on an activated live capture, and the result of the call was discarded, so
  setting it on an inactive, offline or dead capture silently had no effect. Those calls no longer
  compile, and a failure on a live capture is now reported.
...
- The build script no longer assumes libpcap 1.0.0 on Windows when `LIBPCAP_LIBDIR` holds no
  `wpcap.dll`. The Npcap SDK ships the import library in that directory and leaves the DLL to the
  installer, so a build pointed at the SDK had every version-gated binding compiled out. The
  version is now read from the DLL the loader finds by name.
- `immediate_mode` now takes effect on Windows builds that have no `pcap_set_immediate_mode`.
  Those fall back to setting the mintocopy value, which pcap accepts only once the handle has been
  activated, so the call failed and the capture went on buffering. `open` now makes that call once
  `pcap_activate` has returned.
